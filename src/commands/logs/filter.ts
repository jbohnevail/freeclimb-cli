...
import { calculateSinceTimestamp, sleep } from "../../tail"
...
export class logsFilter extends Command {
    static args = [
        { description: "The filter query for retrieving logs.", name: "pql", required: true },
    ]
    ...
    async run() {
        ...
        if (flags.tail) {
            ...
            if (flags.since) {
                ...
                const sinceTimestamp = (() => {
                    try {
                        ...
                        const err = new Errors.SinceFormatError(
                            error as { message: string; name: string }
                        )
                        this.error(err.message, { exit: err.code })
                    }
                })()
            ...
            // eslint-disable-next-line no-await-in-loop -- intentional sequential polling loop
            while (flags.tail) {
