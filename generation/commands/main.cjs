...
const ApiCommand = require("./api-command.cjs")
const mapChars = require("./character-mapping.cjs")
...
import { Command, Flags${(!command.args || command.args.length === 0) && !tail ? "" : ", Args"} } from "@oclif/core"
...
function getAxiosArgs(args, tail) {
    ...
    if ((!args || args.length === 0) && !tail) {
        return data
    }

    data += "\n\tstatic args = {"
    args.forEach((arg) => {
        data += `\t\t${arg.name}: Args.string({description: "${arg.description}", required: ${arg.required}}),\n`
    })
    data += "}\n"
...
function getBooleanParams(booleanParams) {
    ...
    if (booleanParams && booleanParams.length > 0) {
        data += "\n"
