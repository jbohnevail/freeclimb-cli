  ...
  "message": "Response from server"
    ...
    it("Test all required paramaters", async () => {
        nock("https://www.freeclimb.com")
            ...
            .query({})
            .reply(200, testJson)
        const { stdout, stderr } = await runCommand(["call-queues:list"])
    ...
    describe("call-queues:list next flag test", function () {
        it("Tests return of Exit Code 3 when flag next is not available", async () => {
            const orig_FREECLIMB_CALL_QUEUES_LIST_NEXT = process.env.FREECLIMB_CALL_QUEUES_LIST_NEXT
            ...
            try {
                ...
                if (orig_FREECLIMB_CALL_QUEUES_LIST_NEXT === undefined)
                    delete process.env.FREECLIMB_CALL_QUEUES_LIST_NEXT
                else
                    process.env.FREECLIMB_CALL_QUEUES_LIST_NEXT =
                        orig_FREECLIMB_CALL_QUEUES_LIST_NEXT
            }
        })
        ...
        it("Test flag next works as expected when available with on last page", async () => {
            ...
            try {
                const { stdout, stderr } = await runCommand(["call-queues:list", "--next"])
