  ...
  "message": "Response from server"
    ...
    it("Test all required paramaters", async () => {
        nock("https://www.freeclimb.com")
            ...
            .query({})
            .reply(200, testJson)
        const { stdout, stderr } = await runCommand(["calls:get", "userInput-callId"])
    ...
    it("Test parse error gets triggered when there is an additional argument", async () => {
        const { error } = await runCommand(["calls:get", "userInput-callId", "additionalArguments"])
    ...
    describe("calls:get next flag test", function () {
        it("Tests return of Exit Code 3 when flag next is not available", async () => {
            const orig_FREECLIMB_CALLS_GET_NEXT = process.env.FREECLIMB_CALLS_GET_NEXT
            ...
            try {
                ...
                const { error } = await runCommand(["calls:get", "userInput-callId", "--next"])
                expect(error?.oclif?.exit).to.equal(3)
            } finally {
                if (orig_FREECLIMB_CALLS_GET_NEXT === undefined)
                    delete process.env.FREECLIMB_CALLS_GET_NEXT
                else process.env.FREECLIMB_CALLS_GET_NEXT = orig_FREECLIMB_CALLS_GET_NEXT
            }
        })
    })
})
