  ...
  "message": "Response from server"
    ...
    it("Test all required paramaters", async () => {
        nock("https://www.freeclimb.com")
            ...
            .query({})
            .reply(200, testJson)
        const { stdout, stderr } = await runCommand(["accounts:manage"])
    ...
    describe("accounts:manage next flag test", function () {
        it("Tests return of Exit Code 3 when flag next is not available", async () => {
            const orig_FREECLIMB_ACCOUNTS_MANAGE_NEXT = process.env.FREECLIMB_ACCOUNTS_MANAGE_NEXT
            ...
            try {
                ...
                if (orig_FREECLIMB_ACCOUNTS_MANAGE_NEXT === undefined)
                    delete process.env.FREECLIMB_ACCOUNTS_MANAGE_NEXT
                else
                    process.env.FREECLIMB_ACCOUNTS_MANAGE_NEXT = orig_FREECLIMB_ACCOUNTS_MANAGE_NEXT
            }
        })
    })
})
