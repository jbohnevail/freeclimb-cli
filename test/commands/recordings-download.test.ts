...
describe("recordings:download Data Test", function () {
  ...
  "message": "Response from server"
    ...
    it("Test Freeclimb Api error repsonce is process correctly without a suggestion", async () => {
        ...
        const { error } = await runCommand(["recordings:download", "userInput-recordingId"])
    ...
    describe("recordings:download next flag test", function () {
        it("Tests return of Exit Code 3 when flag next is not available", async () => {
            const orig_FREECLIMB_RECORDINGS_DOWNLOAD_NEXT =
                process.env.FREECLIMB_RECORDINGS_DOWNLOAD_NEXT
            ...
            nock("https://www.freeclimb.com")
                ...
                .reply(200, testJson)
            try {
                await runCommand(["recordings:download", "userInput-recordingId"])
                const { error } = await runCommand([
                    "recordings:download",
                    "userInput-recordingId",
                    "--next",
                ...
                if (orig_FREECLIMB_RECORDINGS_DOWNLOAD_NEXT === undefined)
                    delete process.env.FREECLIMB_RECORDINGS_DOWNLOAD_NEXT
                else
                    process.env.FREECLIMB_RECORDINGS_DOWNLOAD_NEXT =
                        orig_FREECLIMB_RECORDINGS_DOWNLOAD_NEXT
            }
        })
    })
})
