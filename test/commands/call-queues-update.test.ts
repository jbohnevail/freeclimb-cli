  ...
  "message": "Response from server"
    ...
    it("Test all required paramaters", async () => {
        nock("https://www.freeclimb.com")
            ...
            .query({})
            .reply(200, testJson)
        const { stdout, stderr } = await runCommand(["call-queues:update", "userInput-queueId"])
    ...
    describe("call-queues:update next flag test", function () {
        it("Tests return of Exit Code 3 when flag next is not available", async () => {
            const orig_FREECLIMB_CALL_QUEUES_UPDATE_NEXT =
                process.env.FREECLIMB_CALL_QUEUES_UPDATE_NEXT
            ...
            try {
                ...
                if (orig_FREECLIMB_CALL_QUEUES_UPDATE_NEXT === undefined)
                    delete process.env.FREECLIMB_CALL_QUEUES_UPDATE_NEXT
                else
                    process.env.FREECLIMB_CALL_QUEUES_UPDATE_NEXT =
                        orig_FREECLIMB_CALL_QUEUES_UPDATE_NEXT
            }
        })
    })
})
