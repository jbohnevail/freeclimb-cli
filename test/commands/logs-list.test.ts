  ...
  "message": "Response from server"
    ...
    it("Test all required paramaters", async () => {
        nock("https://www.freeclimb.com")
            ...
            .query({})
            .reply(200, testJson)
        const { stdout, stderr } = await runCommand(["logs:list"])
    ...
    it("Test parse error gets triggered when there is an additional argument", async () => {
        const { error } = await runCommand(["logs:list", "additionalArguments"])
  ...
  },
  {
    ...
    "message": "this is log two"
        ...
        it("Test all required paramaters and flag maxItem", async () => {
            ...
            const { stdout, stderr } = await runCommand(["logs:list", "--maxItem", "2"])
            expect(stdout).to.contain(nockServerResponseLogs)
        })

        it("Test maxItem with --next", async () => {
            const orig_FREECLIMB_LOGS_LIST_NEXT = process.env.FREECLIMB_LOGS_LIST_NEXT
            ...
            try {
                ...
                if (orig_FREECLIMB_LOGS_LIST_NEXT === undefined)
                    delete process.env.FREECLIMB_LOGS_LIST_NEXT
                else process.env.FREECLIMB_LOGS_LIST_NEXT = orig_FREECLIMB_LOGS_LIST_NEXT
            }
        })
    })
    ...
    describe("logs:list next flag test", function () {
        ...
        it("Test flag next works as expected when available with on last page", async () => {
            ...
            try {
                const { stdout, stderr } = await runCommand(["logs:list", "--next"])
