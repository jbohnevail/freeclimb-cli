  ...
  "message": "Response from server"
    ...
    describe("queue-members:dequeue next flag test", function () {
        it("Tests return of Exit Code 3 when flag next is not available", async () => {
            const orig_FREECLIMB_QUEUE_MEMBERS_DEQUEUE_NEXT =
                process.env.FREECLIMB_QUEUE_MEMBERS_DEQUEUE_NEXT
            ...
            nock("https://www.freeclimb.com")
                ...
                .reply(200, testJson)
            try {
                await runCommand(["queue-members:dequeue", "userInput-queueId", "userInput-callId"])
                const { error } = await runCommand([
                    "queue-members:dequeue",
                    "userInput-queueId",
                    "userInput-callId",
                ...
                if (orig_FREECLIMB_QUEUE_MEMBERS_DEQUEUE_NEXT === undefined)
                    delete process.env.FREECLIMB_QUEUE_MEMBERS_DEQUEUE_NEXT
                else
                    process.env.FREECLIMB_QUEUE_MEMBERS_DEQUEUE_NEXT =
                        orig_FREECLIMB_QUEUE_MEMBERS_DEQUEUE_NEXT
            }
        })
    })
})
