...
describe("incoming-numbers:get Data Test", function () {
  ...
  "message": "Response from server"
    ...
    it("Test Freeclimb Api error repsonce is process correctly without a suggestion", async () => {
        ...
        const { error } = await runCommand(["incoming-numbers:get", "userInput-phoneNumberId"])
    ...
    describe("incoming-numbers:get next flag test", function () {
        it("Tests return of Exit Code 3 when flag next is not available", async () => {
            const orig_FREECLIMB_INCOMING_NUMBERS_GET_NEXT =
                process.env.FREECLIMB_INCOMING_NUMBERS_GET_NEXT
            ...
            nock("https://www.freeclimb.com")
                ...
                .reply(200, testJson)
            try {
                await runCommand(["incoming-numbers:get", "userInput-phoneNumberId"])
                const { error } = await runCommand([
                    "incoming-numbers:get",
                    "userInput-phoneNumberId",
                    "--next",
                ...
                if (orig_FREECLIMB_INCOMING_NUMBERS_GET_NEXT === undefined)
                    delete process.env.FREECLIMB_INCOMING_NUMBERS_GET_NEXT
                else
                    process.env.FREECLIMB_INCOMING_NUMBERS_GET_NEXT =
                        orig_FREECLIMB_INCOMING_NUMBERS_GET_NEXT
            }
        })
    })
})
