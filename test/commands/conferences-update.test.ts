...
describe("conferences:update Data Test", function () {
  ...
  "message": "Response from server"
    ...
    it("Test Freeclimb Api error repsonce is process correctly without a suggestion", async () => {
        ...
        const { error } = await runCommand(["conferences:update", "userInput-conferenceId"])
    ...
    describe("conferences:update next flag test", function () {
        it("Tests return of Exit Code 3 when flag next is not available", async () => {
            const orig_FREECLIMB_CONFERENCES_UPDATE_NEXT =
                process.env.FREECLIMB_CONFERENCES_UPDATE_NEXT
            ...
            nock("https://www.freeclimb.com")
                ...
                .reply(200, testJson)
            try {
                await runCommand(["conferences:update", "userInput-conferenceId"])
                const { error } = await runCommand([
                    "conferences:update",
                    "userInput-conferenceId",
                    "--next",
                ...
                if (orig_FREECLIMB_CONFERENCES_UPDATE_NEXT === undefined)
                    delete process.env.FREECLIMB_CONFERENCES_UPDATE_NEXT
                else
                    process.env.FREECLIMB_CONFERENCES_UPDATE_NEXT =
                        orig_FREECLIMB_CONFERENCES_UPDATE_NEXT
            }
        })
    })
})
