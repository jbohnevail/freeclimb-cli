...
describe("queue-members:dequeue-head Data Test", function () {
  ...
  "message": "Response from server"
    ...
    it("Test Freeclimb Api error repsonce is process correctly without a suggestion", async () => {
        ...
        const { error } = await runCommand(["queue-members:dequeue-head", "userInput-queueId"])
    ...
    describe("queue-members:dequeue-head next flag test", function () {
        it("Tests return of Exit Code 3 when flag next is not available", async () => {
            const orig_FREECLIMB_QUEUE_MEMBERS_DEQUEUE_HEAD_NEXT =
                process.env.FREECLIMB_QUEUE_MEMBERS_DEQUEUE_HEAD_NEXT
            ...
            nock("https://www.freeclimb.com")
                ...
                .reply(200, testJson)
            try {
                await runCommand(["queue-members:dequeue-head", "userInput-queueId"])
                const { error } = await runCommand([
                    "queue-members:dequeue-head",
                    "userInput-queueId",
                    "--next",
                ...
                if (orig_FREECLIMB_QUEUE_MEMBERS_DEQUEUE_HEAD_NEXT === undefined)
                    delete process.env.FREECLIMB_QUEUE_MEMBERS_DEQUEUE_HEAD_NEXT
                else
                    process.env.FREECLIMB_QUEUE_MEMBERS_DEQUEUE_HEAD_NEXT =
                        orig_FREECLIMB_QUEUE_MEMBERS_DEQUEUE_HEAD_NEXT
            }
        })
    })
})
