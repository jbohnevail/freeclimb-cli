  ...
  "message": "Response from server"
    ...
    it("Test all required paramaters", async () => {
        nock("https://www.freeclimb.com")
            ...
            .query({})
            .reply(200, testJson)
        const { stdout, stderr } = await runCommand(["sms:get", "userInput-messageId"])
    ...
    it("Test Freeclimb Api error repsonce is process correctly without a suggestion", async () => {
        ...
        const { error } = await runCommand(["sms:get", "userInput-messageId"])
    ...
    describe("sms:get next flag test", function () {
        it("Tests return of Exit Code 3 when flag next is not available", async () => {
            const orig_FREECLIMB_SMS_GET_NEXT = process.env.FREECLIMB_SMS_GET_NEXT
            ...
            try {
                ...
                const { error } = await runCommand(["sms:get", "userInput-messageId", "--next"])
                expect(error?.oclif?.exit).to.equal(3)
            } finally {
                if (orig_FREECLIMB_SMS_GET_NEXT === undefined)
                    delete process.env.FREECLIMB_SMS_GET_NEXT
                else process.env.FREECLIMB_SMS_GET_NEXT = orig_FREECLIMB_SMS_GET_NEXT
            }
        })
    })
})
