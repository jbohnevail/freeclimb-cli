  ...
  "message": "Response from server"
    ...
    it("Test all required paramaters", async () => {
        nock("https://www.freeclimb.com")
            ...
            .query({})
            .reply(200, testJson)
        const { stdout, stderr } = await runCommand(["recordings:stream", "userInput-recordingId"])
    ...
    it("Test Freeclimb Api error repsonce is process correctly without a suggestion", async () => {
        ...
        const { error } = await runCommand(["recordings:stream", "userInput-recordingId"])
    ...
    describe("recordings:stream next flag test", function () {
        it("Tests return of Exit Code 3 when flag next is not available", async () => {
            const orig_FREECLIMB_RECORDINGS_STREAM_NEXT =
                process.env.FREECLIMB_RECORDINGS_STREAM_NEXT
            ...
            nock("https://www.freeclimb.com")
                ...
                .reply(200, testJson)
            try {
                await runCommand(["recordings:stream", "userInput-recordingId"])
                const { error } = await runCommand([
                    "recordings:stream",
                    "userInput-recordingId",
                    "--next",
                ...
                if (orig_FREECLIMB_RECORDINGS_STREAM_NEXT === undefined)
                    delete process.env.FREECLIMB_RECORDINGS_STREAM_NEXT
                else
                    process.env.FREECLIMB_RECORDINGS_STREAM_NEXT =
                        orig_FREECLIMB_RECORDINGS_STREAM_NEXT
            }
        })
    })
})
