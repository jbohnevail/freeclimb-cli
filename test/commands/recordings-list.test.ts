  ...
  "message": "Response from server"
    ...
    it("Test all required paramaters", async () => {
        nock("https://www.freeclimb.com")
            ...
            .query({})
            .reply(200, testJson)
        const { stdout, stderr } = await runCommand(["recordings:list"])
    ...
    it("Test parse error gets triggered when there is an additional argument", async () => {
        const { error } = await runCommand(["recordings:list", "additionalArguments"])
    ...
    describe("recordings:list next flag test", function () {
        it("Tests return of Exit Code 3 when flag next is not available", async () => {
            const orig_FREECLIMB_RECORDINGS_LIST_NEXT = process.env.FREECLIMB_RECORDINGS_LIST_NEXT
            ...
            try {
                ...
                const { error } = await runCommand(["recordings:list", "--next"])
                expect(error?.oclif?.exit).to.equal(3)
            } finally {
                if (orig_FREECLIMB_RECORDINGS_LIST_NEXT === undefined)
                    delete process.env.FREECLIMB_RECORDINGS_LIST_NEXT
                else
                    process.env.FREECLIMB_RECORDINGS_LIST_NEXT = orig_FREECLIMB_RECORDINGS_LIST_NEXT
            }
        })
        ...
        it("Test flag next works as expected when available with on last page", async () => {
            const orig_FREECLIMB_RECORDINGS_LIST_NEXT = process.env.FREECLIMB_RECORDINGS_LIST_NEXT
            process.env.FREECLIMB_RECORDINGS_LIST_NEXT = "7265636f7264696e67733a6c697374"
            nock("https://www.freeclimb.com")
                .get(`/apiserver/Accounts/${await cred.accountId}/Recordings`)
                .query({ cursor: "7265636f7264696e67733a6c697374" })
                .reply(200, testJsonNext)
            try {
                const { stdout, stderr } = await runCommand(["recordings:list", "--next"])
